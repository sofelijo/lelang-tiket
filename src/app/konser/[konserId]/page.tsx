...
export default function TiketByKonserPage() {
  ...
  const [sortBy, setSortBy] = useState<"populer" | "mauhabis" | "murah">("populer");
  const [hargaMurahMode, setHargaMurahMode] = useState<"total" | "satuan">("total");
  ...
  const tiketTerfilter = tiketList.filter((t) => {
    ...
    if (filterSebelahan !== null && t.sebelahan !== filterSebelahan) return false;
    return true;
  });
  ...
  return (
    <div className="max-w-screen-xl mx-auto flex flex-col lg:flex-row gap-6 px-4 py-6">
      {/* Sidebar Konser */}
      <aside className="lg:w-1/3 space-y-4 sticky top-4 self-start">
        <Card className="overflow-hidden">
          <img src={konser.image} alt="banner konser" className="w-full h-48 object-cover" />
          <div className="p-4 space-y-2">
            <h1 className="text-xl font-bold">{konser.nama}</h1>
            <p className="text-sm text-muted-foreground">📍 {konser.venue}, {konser.lokasi}</p>
            <p className="text-sm">📅 {format(new Date(konser.tanggal), "dd MMMM yyyy")}</p>
          </div>
        </Card>
      </aside>
      ...
      <section className="lg:w-2/3 space-y-4">
        ...
        <div className="flex flex-wrap gap-3 items-center">
          ...
          {[true, false].map((val) => (
            <Badge
              key={String(val)}
              onClick={() => setFilterSebelahan(filterSebelahan === val ? null : val)}
              className={cn(
                "cursor-pointer",
                filterSebelahan === val
          ...
          {sortBy === "murah" ? (
            <select
              ...
              onChange={(e) => setHargaMurahMode(e.target.value as "total" | "satuan")}
            >
  ...
  <div className="w-[70px] text-right">Aksi</div>
</Card>
        ...
        <div className="space-y-3">
          {tiketTerfilter.map((tiket) => {
            const hargaTotal = tiket.metode === "LELANG"
            ...
            const jamSisa = tiket.batas_waktu
              ? (new Date(tiket.batas_waktu).getTime() - Date.now()) / (1000 * 60 * 60)
              : null;
            ...
            if (jamSisa !== null) {
              ...
              else if (jamSisa <= 3) waktuClass = "text-red-500 animate-blink-slow";
              else if (jamSisa <= 6) waktuClass = "text-red-500";
              else if (jamSisa <= 12) waktuClass = "text-yellow-500";
            }

            return (
              <Card key={tiket.id} className="p-4 flex items-center justify-between gap-4">
  ...
  Rp {hargaTotal.toLocaleString()}
                  ...
                  <div className={cn("w-[100px] font-medium", waktuClass)}>{waktu}</div>
