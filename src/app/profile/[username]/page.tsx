...
export default function PublicProfilePage() {
  ...
  useEffect(() => {
    if (!loadMoreRef.current || visibleTickets.length >= filtered.length)
      return;
      ...
      <div className="md:col-span-2 space-y-4">
        ...
        {loading ? (
          Array.from({ length: 3 }).map((_, i) => (
            <Card
              key={i}
              className="p-4 flex flex-col md:flex-row gap-4 bg-blue-50/40 border-blue-100"
            >
          ...
          <p className="text-sm text-muted-foreground">
            Belum ada tiket untuk status ini 👀
          </p>
        ) : (
          visibleTickets.map((t: any, i: number) => (
           

<Card key={i} className="p-4 flex flex-col md:flex-row gap-4 border shadow bg-white">
  ...
  <div className="flex-1 flex flex-col justify-between space-y-1">
    <div className="space-y-1">
      ...
      <p className="text-xs text-muted-foreground">
        🗓️ {format(new Date(t.konser.tanggal), "dd MMM yyyy")} | 📍 {t.konser.lokasi}
      </p>
      ...
      <div className="flex flex-wrap gap-2 text-xs mt-2">
        ...
        <Badge variant="outline">
          {t.sebelahan ? "👯‍♂️ Sebelahan" : "🔀 Acak"}
        </Badge>
        <Badge variant="outline">🎟️ {t.jumlah}</Badge>

        {/* Badge Status Lelang */}
        {(() => {
          const pembayaranBerhasil = t.pembayaran?.find(
            (p: any) => p.statusPembayaran === "BERHASIL"
          );
          const sudahDipakai = pembayaranBerhasil?.sudahDipakai === true;
          const isRefund = t.pembayaran?.some(
            (p: any) => p.statusPembayaran === "REFUND"
          );
          const belumBayar = !pembayaranBerhasil;

          let label = t.statusLelang;
          let bg = "bg-slate-500 hover:bg-slate-600";

          if (t.statusLelang === "SELESAI") {
            if (isRefund) {
              label = "❌ REFUND";
              bg = "bg-red-600 hover:bg-red-700";
            } else if (pembayaranBerhasil && sudahDipakai) {
              label = "✅ SELESAI";
              bg = "bg-green-600 hover:bg-green-700";
            } else if (pembayaranBerhasil && !sudahDipakai) {
              label = "🕓 SELESAI (Belum Dipakai)";
              bg = "bg-yellow-600 hover:bg-yellow-700";
            } else if (belumBayar) {
              label = "💸 SELESAI (Belum Dibayar)";
              bg = "bg-slate-500 hover:bg-slate-600";
            }
          } else if (t.statusLelang === "BERLANGSUNG") {
            label = "🔥 BERLANGSUNG";
        ...
        {visibleTickets.length < filtered.length && (
          <div ref={loadMoreRef} className="space-y-2 animate-pulse">
            {Array.from({ length: 2 }).map((_, i) => (
              <Card
                key={i}
                className="p-4 flex flex-col md:flex-row gap-4 bg-pink-50/50 border-pink-200"
              >
                <Skeleton className="h-24 w-full md:w-40 rounded-md" />
                <div className="flex-1 space-y-2">
                  <Skeleton className="h-4 w-2/3" />
